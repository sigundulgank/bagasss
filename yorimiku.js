...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const totalFitur = () =>{
            var mytext = fs.readFileSync("./yorimiku.js").toString()
    ...
    const _0x547672 =['8SFTvTY','733698bByGMZ','1nqbVzC','11dOuxIL','chat','1838816QQlKFJ','992463nstgLu','4616415lmcnqf','1014PlYMnG','23639900whcpis','5450zYeATk','1020716qbYiGA'];_0x1aa5=function(){return _0x547672;};return _0x1aa5();}
        ...
        if (db.data.chats[m.chat].antivirtex) {
            if (budy.length > 3500) {
                ...
                if (!isBotAdmins) return newReply(`Ehh Bot Gak Admin T_T`)
                if (isAdmins) return newReply(`Ehh Maaf Ternyata Kamu Admin 😁`)
                if (isCreator) return newReply(`Ehh Maaf Kamu Yori Hosting Ownerku Ternyata 😅`)
                conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
            }
        }
...
function _0x35fb() {
    var _0x4f1e3b = ['https://youtube.com/playlist?list=PLRmLRCoejWJsauHXGmkKERWnptlwtOy2q', '2070kRGErB', '28295pqllNW', '531tjwrrz', '1079yZujew', '906Wuppor', '2256950dFlorC', '44XNZJgv', '9420776hAoMmA', '2743524iQbgfX', '28568rvnzvB'];
    _0x35fb = function () {
        return _0x4f1e3b;
    };
    return _0x35fb();
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
            ...
            case 'gimage': {
                if (!text) return newReply(`Contoh : ${prefix + command} Yori Hosting`)
                newReply(mess.wait)
                if (!isPremium && global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 1 // -1 limit
		...
		case 'brainly':{
            ...
            case 'attp':
                try {
                if (args.length == 0) return newReply(`Example: ${prefix + command} Yori Hosting`)
            ...
            let media = await conn.downloadMediaMessage(qmsg)
            let audio = await toAudio(media, 'mp4')
            conn.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By Yori Hosting .mp3`}, { quoted : m })
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By Yori Hosting\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Yori Hosting\n\n Type: ${command}\n\nText: ${args}`})
			break
...
Lihat list Pesan Dengan ${prefix}listmsg`)
            ...
            */
            ...
            case 'allmenu': 
            case 'menu': {
    let anu =`Hello ${pushname}\nI Am ${namabot} Is a WhatsApp Bot That Has Features And I'm Just Here To Make You Happy With Our Features.

  ☫ *Mode:* ${conn.public ? 'Public-Mode 👥' : 'Self-Mode 👤'}
  ☫ *Bailyes:* YoriHosting/Baileys
