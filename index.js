...
async function startAdrian() {
    ...
    const conn = connConnect({
        ...
        browser: ['YoriMiku Multi Device','Safari','1.0.0'],
   ...
   // anticall auto block
    ...
    conn.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./yorimiku")(conn, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
	...
	conn.ev.on("groups.update", async (json) => {
				...
				conn.sendMessage(res.id, {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            ...
                            sourceUrl: "https://instagram/@yorimikuxd",
                            mediaType: 1,
        ...
        try {
            ...
            for (let num of participants) {
               ...
               if (anu.action == 'add') {
                 let a = `*Hello @${num.split("@")[0]}, Welcome To ${metadata.subject}*\n, I congratulate you on joining and don't forget to read the description😊`
     ...
     text: a, 
      contextInfo: {
         ...
         renderLargerThumbnail: true
                    ...
                    let a = `See you soon, and don't forget to come back later😊`
      conn.sendMessage(anu.id, {
     text: a, 
      contextInfo: {
         externalAdReply: {
